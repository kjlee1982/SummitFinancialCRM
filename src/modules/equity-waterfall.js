...
export const equityWaterfall = {
  ...
  /**
   * Renders the Equity Waterfall tab.
   *
   * main.js expects modules to expose a render(state) function.
   */
  render(state) {
    ...
    if (!this._selectedDealId) {
      const saved = sessionStorage.getItem('equityWaterfall.selectedDealId');
      if (saved) this._selectedDealId = saved;
    }

    // If none selected, try the first deal
    if (!this._selectedDealId && deals.length) {
      this._selectedDealId = String(deals[0].id);
    }

    const options = deals
      .map(d => {
        const id = String(d?.id ?? '');
        const name = escapeHtml(d?.name ?? 'Untitled Deal');
        const sel = String(this._selectedDealId) === id ? 'selected' : '';
        return `<option value="${escapeHtml(id)}" ${sel}>${name}</option>`;
      })
      .join('');
      ...
      <div class="p-6 max-w-6xl mx-auto">
        <div class="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm">
          <div class="flex flex-col md:flex-row md:items-end md:justify-between gap-4">
            <div>
              <h2 class="text-2xl font-black italic tracking-tight text-slate-900">Equity Waterfall</h2>
              <p class="text-sm font-semibold text-slate-500 mt-1">Select a deal to view a simplified pref + promote waterfall.</p>
            </div>

            <div class="w-full md:w-[360px]">
              <label class="block text-xs font-black text-slate-600 uppercase tracking-widest mb-2">Deal</label>
              <select id="equityWaterfallDealSelect" class="w-full px-3 py-2 rounded-xl border border-slate-200 bg-white text-slate-900 font-semibold">
                ${options || '<option value="">No deals available</option>'}
              </select>
            </div>
          </div>

          <div id="equity-waterfall-container" class="mt-6"></div>
        </div>
      </div>
    `;
    ...
    if (!this._bound) {
      ...
      document.addEventListener('change', (e) => {
        const sel = e.target?.id === 'equityWaterfallDealSelect' ? e.target : null;
        if (!sel) return;
        this._selectedDealId = String(sel.value || '');
        sessionStorage.setItem('equityWaterfall.selectedDealId', this._selectedDealId);
        this.renderWaterfallUI(this._selectedDealId, state, 'equity-waterfall-container');
      });
    }
    ...
    if (this._selectedDealId) {
      ...
      const container = document.getElementById('equity-waterfall-container');
      if (container) {
        container.innerHTML = `
          <div class="p-6 bg-white rounded-xl border border-gray-200 text-gray-500">
            <p class="font-bold">No deals found.</p>
            <p class="text-sm">Create a deal first, then come back to view the waterfall.</p>
          </div>
        `;
      }
    }
